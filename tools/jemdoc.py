...
def replacelinks(b):
  ...
  while m:
    ...
    external_link = link.startswith('http://') or link.startswith('https://')
    link = quote(link)
    ...
    if external_link:
      attrs = r' href=\"%s\" target=\"_blank\"' % link
    else:
      attrs = r' href=\"%s\"' % link

    b = b[:m.start()] + r'<a%s>%s<\/a>' % (attrs, linkname) + b[m.end():]
    ...
    # look for blocks.
      ...
      if infoblock:
        ...
        if len(g) in (0, 1): # info block.
          ...
          out(f.outf, '<img class="profile-photo" src="%s"' % g[2])
